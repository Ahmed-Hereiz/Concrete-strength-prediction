...
def train_models(X_train, X_test, y_train, y_test):
    ...
    for name, model in models.items():
        ...
        plt.savefig(f'saved/assets/{name.replace(" ", "_").lower()}_shap_summary.png', bbox_inches='tight')
        plt.close()
        
        plt.figure(figsize=(10, 6))
        shap.summary_plot(shap_values, X_test, plot_type="bar", show=False, plot_size=(10, 6))
        plt.title(f'{name} Mean SHAP Value')
        plt.savefig(f'saved/assets/{name.replace(" ", "_").lower()}_shap_bar.png', bbox_inches='tight')
        plt.close()
        ...
        # Use the training data to initialize the Lime explainer.
        lime_explainer = LimeTabularExplainer(
            X_train.values,
            feature_names=X_train.columns,
            mode='regression'
        )
        # Select a single instance from X_test to explain (for example, the first instance).
        instance_idx = 0
        instance = X_test.iloc[instance_idx]
        explanation = lime_explainer.explain_instance(instance.values, model.predict, num_features=10)
        
        # Save LIME explanation as a matplotlib figure.
        fig = explanation.as_pyplot_figure()
        plt.title(f"{name} LIME Explanation (Instance {instance_idx})")
        plt.savefig(f'saved/assets/{name.replace(" ", "_").lower()}_lime_explanation.png', bbox_inches='tight')
        plt.close()
        
        # ---------------------------
        # Permutation Feature Importance
        # ---------------------------
        print(f"Generating Permutation Feature Importance for {name}...")
        ...
        plt.tight_layout()
        plt.savefig(f'saved/assets/{name.replace(" ", "_").lower()}_permutation_importance.png', bbox_inches='tight')
        plt.close()
...
if __name__ == "__main__":
    ...
    print("Training models and generating explanations...")
